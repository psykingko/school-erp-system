    ...
    <MainCard className="p-6 border border-slate-100 bg-white shadow-sm rounded-3xl h-full flex flex-col justify-between overflow-visible relative">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-4 border-b border-slate-100 mb-5 gap-4">
          <div>
            <span className="text-[9px] font-black text-blue-600 uppercase tracking-wider bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-md">
              {t("teacherDashboard.teachingMovement", { fallback: "Teaching Movement" })}
            </span>
            <h3 className="text-base font-black text-[#03045e] mt-1.5 flex items-center gap-2">
              <Compass className="w-5 h-5 text-blue-600" />
              {t("teacherDashboard.myTeachingSchedule", { fallback: "My Teaching Schedule" })} {isToday ? `(${t("teacherDashboard.today", { fallback: "Today" })})` : `(${String(selectedDate.getDate()).padStart(2, '0')}/${String(selectedDate.getMonth() + 1).padStart(2, '0')}/${selectedDate.getFullYear()})`}
            </h3>
          </div>
          ...
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-black bg-slate-50 text-slate-600 px-3 py-1.5 rounded-full border border-slate-100 hidden sm:block">
              {displaySchedule.length} {t("teacherDashboard.lectures", { fallback: "Lectures" })}
            </span>
            <div className="flex bg-blue-50/50 p-1 rounded-xl border border-blue-50">
              <button 
                ...
                title={t("teacherDashboard.prevDay", { fallback: "Previous Day" })}
              >
                <ChevronLeft size={17} />
              </button>
              <button 
                ...
                className="p-1.5 hover:bg-white rounded-lg transition-all text-blue-600"
                title={t("teacherDashboard.nextDay", { fallback: "Next Day" })}
              >
            ...
            <div className="relative">
              <button 
                ...
                title={t("teacherDashboard.selectDate", { fallback: "Select Date" })}
              >
        ...
        {displaySchedule.length === 0 ? (
          <div className="text-center py-12 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-xs font-bold text-slate-400 italic">
              {isToday ? t("teacherDashboard.noTeachingPeriodsToday", { fallback: "No teaching periods assigned to you today." }) : t("teacherDashboard.noTeachingPeriodsDate", { fallback: "No teaching periods scheduled on this date." })}
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-xs text-slate-600">{t("teacherDashboard.class", {fallback: "Class"})} {item.class}</span>
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    {isCurrent && (
                      <span className="flex items-center gap-1 text-[8px] font-black bg-emerald-500 text-white px-2 py-0.5 rounded-full uppercase tracking-widest">
                        ...
                        {t("teacherDashboard.activeNow", {fallback: "Active Now"})}
                      </span>
                    )}
                    {isNext && !isCurrent && (
                      <span className="flex items-center gap-1 text-[8px] font-black bg-amber-500 text-white px-2 py-0.5 rounded-full uppercase tracking-widest">
                        {t("teacherDashboard.nextUp", {fallback: "Next Up"})}
                      </span>
                    )}
                  </div>
                </div>
      ...
      {displaySchedule.length > 0 && (
        <div className="pt-4 border-t border-slate-100 mt-6 flex items-center gap-2 text-[10px] font-bold text-slate-400">
          ...
          <span>{t("teacherDashboard.reportConflicts", {fallback: "Need adjustments? Report schedule conflicts to the Coordination Desk."})}</span>
        </div>
      )}
    </MainCard>
  );
