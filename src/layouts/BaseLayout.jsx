...
const BaseLayout = React.memo(({ children, navItems = [], activePage, setActivePage, notifications = [], currentDate }) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#caf0f8]">
      ...
      <div
        className={`flex-1 flex flex-col min-w-0 transition-[margin] duration-300 ml-0 md:ml-16 ${sidebarCollapsed ? "lg:ml-16" : "lg:ml-60"} print:!m-0 print:!block print:w-full`}
      >
        <Header
          student={user}
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-4 md:py-6 lg:py-8 w-full max-w-full print:p-0 print:m-0 print:block">
          {children || <Outlet />}
        </main>
      </div>
